...
import { BUSD, USDC, CAKE } from 'config/constants/tokens'
...
export default function useBUSDPrice(currency?: Currency): Price<Currency, Currency> | undefined {
  ...
  const wnative = WNATIVE[chainId] || WNATIVE[ChainId.ETHEREUM]
  const busd = BUSD[chainId] || BUSD[ChainId.ETHEREUM]
...
export const useCakeBusdPrice = (): Price<Currency, Currency> | undefined => {
  ...
  const cake: Token = CAKE[chainId] ? CAKE[chainId] : CAKE[ChainId.ETHEREUM]
